...
void setup() {
  ...
  table.addColumn("tecnica");
  table.addColumn("tipo de movimiento");
  size(1650, 902);
  noStroke();
  frameRate(40);
...
void draw() {
tipoDeMovimiento= cp5.get(Textfield.class,"TIPO DE MOVIMIENTO").getText();
tecnica= cp5.get(Textfield.class,"TECNICA").getText();
  ...
  if (valor4 >= 0.1) {
    ...
    if(GRABAR) {
  guardarData();
println("ENVIANDO DATOS A EXCEL");}
  popMatrix();
if(camara.available()){
...
void guardarData() {
  ...
  newRow.setString("hora", messageTime);
    newRow.setString("tecnica", tecnica);
      newRow.setString("tipo de movimiento", tipoDeMovimiento);
